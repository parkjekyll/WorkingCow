			...
			<div class="main_detail">
				<table class="detail">
					<tr >
						...
						<th style="width: 20%;">문의상품명</th>
						<th style="width: 20%;">문의제목</th>
						
						<th style="width: 20%;">등록날짜</th>
						<th style="width: 15%;">답변여부</th>
						<th style="width: 25%;">답변보기</th>
					</tr>	
					<c:forEach items="${searchInquiryList}" var="data">
					<tr>
					
						<td style="width: 10%;">${data.inquiryToSellerVO.inquiry_no }</td>
						<td style="width: 20%;">${data.productVO.product_title }</td>
						<td style="width: 20%;"><a href="${pageContext.request.contextPath}/readInquiryContent.do?inquiry_no=${data.inquiryToSellerVO.inquiry_no }">${data.inquiryToSellerVO.inquiry_title }</a></td>
						<td style="width: 20%;"><fmt:formatDate value="${data.inquiryToSellerVO.inquiry_date }"/></td>
						<td style="width: 15%;">${data.inquiryToSellerVO.inquiry_status }</td>
						<c:if test="${data.inquiryToSellerVO.inquiry_status eq 'Y'.charAt(0)}">
						<td style="width: 25%;"><button onclick="location.href='${pageContext.request.contextPath }/readReplyContent.do?inquiry_no=${data.inquiryToSellerVO.inquiry_no }'" type="button" class="btn btn-primary btn-sm">답변보기</button></td>
						</c:if>
				...
				<form action="${pageContext.request.contextPath }/inquiryList.do" method="get">
				<select name="search_type">
					<option value="inquiry_title">제목</option>
					<option value="inquiry_content">내용</option>
				</select>
					<input name="search_word" type="text"><button class="btn btn-primary btn" type="submit">검색</button>
				</form>
					...
					<c:if test="${page > 5 }">
						<a href="inquiryList.do?sub=4&page=${startPage-5 }&fromDate=${param.fromDate }&toDate=${param.toDate }&search_word=${param.search_word}"> ‹ </a>
						</c:if>
					<c:if test="${page < 6 && page > 1}">
						<a href="inquiryList.do?sub=4&page=1&fromDate=${param.fromDate }&toDate=${param.toDate }&search_word=${param.search_word}"> ‹ </a>
					</c:if>
					<c:forEach begin="${startPage }" end="${endPage }" var="i">
						...
						<c:if test="${page ne i }">
							<a href="inquiryList.do?sub=4&page=${i }&fromDate=${param.fromDate }&toDate=${param.toDate }&search_word=${param.search_word}"><span>${i }</span></a>
						</c:if>
					</c:forEach>
					...
					<c:if test="${startPage+5 > totalPage && page < totalPage}">
						<a href="inquiryList.do?sub=4&page=${totalPage }&fromDate=${param.fromDate }&toDate=${param.toDate }&search_word=${param.search_word}"> › </a>
					</c:if>
				</div>		
