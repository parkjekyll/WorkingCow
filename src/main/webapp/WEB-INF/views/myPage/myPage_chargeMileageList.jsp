...
<body>
	...
	<div class="container">
		...
		<div class="main">
			...
			<hr>
			<!-- 날짜로 검색 -->
			<form name="searchFrm" action="myPage_chargeMileageList?sub=7" method="get">
				<input type="text" id="fromDate" name="fromDate" > &nbsp;&nbsp;
				<input type="text" id="toDate" name="toDate" >
				<button type="submit" class ="btn btn-primary"  id="btnSearch">조회</button>
				<button type="button" class ="btn btn-primary" onclick="location.href='./myPage_chargeMileageList?sub=7'">초기화</button>
			</form>
			<hr>
			...
			<div class="main_detail">
				<table class="detail">
					<tr>
						...
						<th style="width: 30%;">마일리지 적립 및 사용</th>
						<th style="width: 30%;">총 마일리지</th>
					</tr>
					<c:set value="1" var="i" />
					<c:if test="${page > 1}"><c:set value="${(page-1)*10+1 }" var="i" /></c:if>
					<c:forEach items="${mileageList }" var="list">
					<c:forEach begin="${i }" end="${i }">
					<tr>
						<td style="width: 10%;">${i }</td>
						<td style="width: 30%;"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${list.mileage_date }"/></td>
						<td style="width: 30%;">
						<c:if test="${list.mileage_add ne 0 }">+ <fmt:formatNumber value="${list.mileage_add }" /> M</c:if>
						<c:if test="${list.mileage_usage ne 0 }">- <fmt:formatNumber value="${list.mileage_usage }" /> M</c:if>
						<c:if test="${list.mileage_usage eq 0 && list.mileage_add eq 0}">+ 0 M</c:if>
						</td>
						<td style="width: 30%;"><fmt:formatNumber value="${list.mileage_amount }" /> M</td>
					</tr>
			...
			<div class="paging">
					...
					<c:if test="${page > 5 }">
						<a href="myPage_chargeMileageList?sub=7&page=${startPage-5 }&fromDate=${param.fromDate }&toDate=${param.toDate }"> ‹ </a>
						</c:if>
					<c:if test="${page < 6 && page > 1}">
						<a href="myPage_chargeMileageList?sub=7&page=1&fromDate=${param.fromDate }&toDate=${param.toDate }"> ‹ </a>
					</c:if>
					<c:forEach begin="${startPage }" end="${endPage }" var="i">
						...
						<c:if test="${page ne i }">
							<a href="myPage_chargeMileageList?sub=7&page=${i }&fromDate=${param.fromDate }&toDate=${param.toDate }"><span>${i }</span></a>
						</c:if>
					</c:forEach>
					...
					<c:if test="${startPage+5 > totalPage && page < totalPage}">
						<a href="myPage_chargeMileageList?sub=7&page=${totalPage }&fromDate=${param.fromDate }&toDate=${param.toDate }"> › </a>
					</c:if>
				</div>
		</div>
	</div>
