	...
	<div class="container">
		...
		<div class="main">
			<div class="main_menudetail">
				<h3>마일리지 관리</h3>
				<form action="admin_mileageManagement" method="get">
			...
			<div class="main_detail">
				<table class="detail">
					<tr>
						...
						<th style="width: 250px;">보유 마일리지</th>
						<th style="width: 600px;">마일리지 추가</th>
					</tr>
					<c:forEach items="${customerList }" var="list">
					<c:forEach items="${result }" var="result">
					<c:if test="${result.mileageVO.customer_wallet_no eq list.customer_wallet_no }">
					<form action="admin_mileageManagement" method="post">
					<tr>
						<td style="width: 159px;">${list.customer_no }</td>
						<td style="width: 200px;">${list.customer_nick }</td>
						<td style="width: 330px;">${list.customer_email }</td>
							<td style="width: 250px;"><fmt:formatNumber value="${result.mileageVO.mileage_amount }" /> M</td>
						<td style="width: 600px;">
						<button class ="btn btn-primary">지급</button>
						<input type="text" name="mileage_give" placeholder="금액을 입력하세요." required="required"> 
						<input type="hidden" name="mileage_amount" value="${result.mileageVO.mileage_amount }"> 
						<input type="hidden" name="customer_wallet_no" value="${list.customer_wallet_no }"> 
						</td>
					</tr>
					</form>
					</c:if>
			...
			<div class="paging">
					...
					<c:if test="${page > 5 }">
						<a href="admin_mileageManagement?sub=4&page=${startPage-5 }"> ‹ </a>
						</c:if>
					<c:if test="${page < 6 && page > 1}">
						<a href="admin_mileageManagement?sub=4&page=1"> ‹ </a>
					</c:if>
					<c:forEach begin="${startPage }" end="${endPage }" var="i">
						...
						<c:if test="${page ne i }">
							<a href="admin_mileageManagement?sub=4&page=${i }"><span>${i }</span></a>
						</c:if>
					</c:forEach>
					...
					<c:if test="${startPage+5 > totalPage && page < totalPage}">
						<a href="admin_mileageManagement?sub=4&page=${totalPage }"> › </a>
					</c:if>
				</div>
		</div>
	</div>
