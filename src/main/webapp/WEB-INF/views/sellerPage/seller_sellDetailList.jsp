...
<body>
	...
	<div class="container">
		...
		<div class="main">
			...
			<hr>
			<!-- 날짜로 검색 -->
			<form name="searchFrm" action="seller_sellDetailList" method="get">
				<input type="text" id="fromDate" name="fromDate" > &nbsp;&nbsp;
				<input type="text" id="toDate" name="toDate" >
				<button type="submit" class ="btn btn-primary"  id="btnSearch">조회</button>
				<button type="button" class ="btn btn-primary" onclick="location.href='seller_sellDetailList?sub=3'">초기화</button>
			</form>
			<hr>
			<div class="main_detail">
				<table class="detail">
					<tr>
						...
						<th style="width: 20%;">판매 상품명</th>
						<th style="width: 20%;">판매일시</th>
						<th style="width: 20%;">판매 금액</th>
					</tr>
					<c:forEach items="${data }" var="data">
					<tr>
						<td style="width: 20%;">${data.customerVO.customer_nick }</td>
						<td style="width: 20%;"><a href="${pageContext.request.contextPath }/product_productDetail?bno=${data.orderVO.product_no}">${data.productVO.product_title }</a></td>
						<td style="width: 20%;"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${data.orderVO.order_date }"/></td>
						<td style="width: 20%;"><fmt:formatNumber value="${data.orderVO.totalPrice }"/> 원</td>			
					</tr>
					</c:forEach>
				</table>
			...
			<div class="paging">
					...
					<c:if test="${page > 5 }">
						<a href="seller_sellDetailList?sub=3&page=${startPage-5 }&fromDate=${param.fromDate }&toDate=${param.toDate }"> ‹ </a>
						</c:if>
					<c:if test="${page < 6 && page > 1}">
						<a href="seller_sellDetailList?sub=3&page=1&fromDate=${param.fromDate }&toDate=${param.toDate }"> ‹ </a>
					</c:if>
					<c:forEach begin="${startPage }" end="${endPage }" var="i">
						...
						<c:if test="${page ne i }">
							<a href="seller_sellDetailList?sub=3&page=${i }&fromDate=${param.fromDate }&toDate=${param.toDate }"><span>${i }</span></a>
						</c:if>
					</c:forEach>
					...
					<c:if test="${startPage+5 > totalPage && page < totalPage}">
						<a href="seller_sellDetailList?sub=3&page=${totalPage }&fromDate=${param.fromDate }&toDate=${param.toDate }"> › </a>
					</c:if>
				</div>
		</div>
	</div>
