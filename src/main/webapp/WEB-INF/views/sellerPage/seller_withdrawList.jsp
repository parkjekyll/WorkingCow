...
<body>
	...
	<div class="container">
		...
		<div class="main">
			...
			<hr>
			<!-- 날짜로 검색 -->
			<form name="searchFrm" action="seller_withdrawList" method="get">
				<input type="text" id="fromDate" name="fromDate" > &nbsp;&nbsp;
				<input type="text" id="toDate" name="toDate" >
				<button type="submit" class ="btn btn-primary"  id="btnSearch">조회</button>
				<button type="button" class ="btn btn-primary" onclick="location.href='seller_withdrawList?sub=6'">초기화</button>
			</form>
			<hr>
			...
			<div class="main_detail">
				<table class="detail">
					<tr>
						<th style="width: 20%;">출금날짜</th>
						<th style="width: 20%;">출금 요청금액</th>
						<th style="width: 20%;">출금 후 잔액</th>
						<th style="width: 20%;">은행</th>
						<th style="width: 20%;">계좌번호</th>
					</tr>
					<c:forEach items="${data }" var="data">
					<tr>
						<td style="width: 20%;"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${data.withdrawVO.withdraw_dateTime }"/></td>
						<td style="width: 20%;">${data.withdrawVO.withdraw_amount } WC</td>
						<td style="width: 20%;">${data.withdrawVO.withdraw_balanceAfWithdraw } WC</td>
						<td style="width: 20%;">${data.withdrawVO.withdraw_bankType }</td>			
						<td style="width: 20%;">${data.withdrawVO.withdraw_account }</td>			
					</tr>
					</c:forEach>
				</table>
			...
			<div class="paging">
					...
					<c:if test="${page > 5 }">
						<a href="seller_withdrawList?sub=6&page=${startPage-5 }&fromDate=${param.fromDate }&toDate=${param.toDate }"> ‹ </a>
						</c:if>
					<c:if test="${page < 6 && page > 1}">
						<a href="seller_withdrawList?sub=6&page=1&fromDate=${param.fromDate }&toDate=${param.toDate }"> ‹ </a>
					</c:if>
					<c:forEach begin="${startPage }" end="${endPage }" var="i">
						...
						<c:if test="${page ne i }">
							<a href="seller_withdrawList?sub=6&page=${i }&fromDate=${param.fromDate }&toDate=${param.toDate }"><span>${i }</span></a>
						</c:if>
					</c:forEach>
					...
					<c:if test="${startPage+5 > totalPage && page < totalPage}">
						<a href="seller_withdrawList?sub=6&page=${totalPage }&fromDate=${param.fromDate }&toDate=${param.toDate }"> › </a>
					</c:if>
				</div>
		</div>
	</div>
